...
const UpdatePeopleToInvitation: FunctionComponent<
  UpdatePeopleToInvitationProps
> = ({ eventId, refresh, onHide, phoneNum }) => {
  let [people, setpoepole] = useState<People>({
    phoneNumber: "",
    firstName: "",
    lastName: "",
    NumberOfGuests: 0,
  ...
  let formik = useFormik({
    initialValues: {
      ...
      NumberOfGuests: people?.NumberOfGuests,
      NumberOfGuestsAccept: people?.NumberOfGuestsAccept,
      eventGroupName: people?.eventGroupName,
    },
    ...
    onSubmit: (values: People) => {
      updatePeopleInEvent(eventId, values)
        .then((res) => {
          onHide();
    ...
    <>
      <div>
        <h5 className='display-5'>
          {" "}
          Update Guost{`=> ${people.firstName} ${people.lastName}`}
        </h5>
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className='form-floating mb-3'>
            <span>
              ...
              <select
                ...
                {allGroup.map((groupname: Group) => (
                  <option
                    key={counter++}
                    value={groupname.eventGroupName}>
                    {" "}
          ...
          <div className='form-floating mb-3'>
            <input
              ...
              onBlur={formik.handleBlur}
            />
            <label htmlFor='NumberOfGuestsAccept'>
              Number Of Guests Accept
            </label>
            {formik.touched.NumberOfGuestsAccept &&
              formik.errors.NumberOfGuestsAccept && (
                <small className='text-danger'>
                  {formik.errors.NumberOfGuestsAccept}
                </small>
              )}
          ...
          >
            <i className='fa-solid fa-floppy-disk'></i> Save Change
          </button>
          ...
          <button
            ...
            <i className='fa-solid fa-comment-sms'></i> Send SMS
          </button>
        </form>
      </div>
      <GloabSeclModal
        show={openSecModal}
        onHide={() => setOpenSecModal(false)}
      />
    </>
  );
};
